...
// viewBox only (no width/height) so CSS controls the size everywhere it's used.
import { PARTS } from "./parts.js";

const svg = (inner) => `<svg viewBox="0 0 40 56" class="part-svg">${inner}</svg>`;
...
export function partArt(id) {
  return ART[id] || svg(`<rect x="10" y="10" width="20" height="36" rx="4" fill="#c7ccd6"/>`);
}
